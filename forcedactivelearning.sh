...
#Forced Iteration Direction
Numb= wc -l rel.csv | awk '{print $1}'

#Declaring a flag to get into a force Iteration
Status='DONE'
Decision=$(awk 'FNR==2 {print $1}' output)

value=$(awk 'FNR==1 {print $1}' output)
#converting Index from a string to integer
value=${value#0}

if [[ $Decision == $Status ]] && [[ $Numb == $2 ]];
then
        ##Making Forced Iteration Directory
        mkdir Forced
        #copying the forcefield, CIF and input files (simulation package) to the next simulation folder
        cp *.def *.cif *.input CompleteData.csv Prior.csv GP.py ActiveLearning.sh Forced
        #changing current directory to next simulation point
        cd Forced

        #changing the placeholder to Pressure
        sed -i 's/XXX/'${value}'/' simulation.input
        #removing the brackets from input file
        sed 's/[][]//g' simulation.input
        #### ----------- Conducting the simulation ------------ ####
        #declaring variables for RASPA
        ...
        mkdir Reformed
        cd Forced
        cp Prior.csv ../Reformed
        cd ../

        cp *.def *.cif *.input CompleteData.csv GP.py ActiveLearning.sh Reformed
        cd Reformed
        mv ActiveLearning.sh reformlearner.sh
        #qsub reformlearner.sh
        else
                #In case If doesn't satisfy, (which means the uncertainty is lower than 2% for all points), break out of this loop and finish Active learning, the model is ready
                break
 fi
