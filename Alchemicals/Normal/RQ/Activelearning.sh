Author: Osaro Etinosa
Advisor: Yamil J Colon
...
do
        ...
        Index=${Index#0}
        #### -------- Creating a Variable to replace the Epsilon value in force_field_mixing_rules.def file ----------###
        YYY=$(grep 'AAA'  ~/RASPA/simulations/share/raspa/forcefield/AlchemicalFF/force_field_mixing_rules.def | awk '{print $3}')
        ZZZ=$(grep 'BBB'  ~/RASPA/simulations/share/raspa/forcefield/AlchemicalFF/force_field_mixing_rules.def | awk '{print $4}')
        ...
        then
                ...
                sed -i 's/XXX/'${Fugacity}'/' simulation.input
                sed -i 's/'${YYY}'/'${Epsilon}'/' ~/RASPA/simulations/share/raspa/forcefield/AlchemicalFF/force_field_mixing_rules.def
                sed -i 's/'${ZZZ}'/'${Siggma}'/' ~/RASPA/simulations/share/raspa/forcefield/AlchemicalFF/force_field_mixing_rules.def
                ...
                rm -f sample.txt
                touch sample.txt
                grep -F 'Average loading absolute [mol/kg framework]' *.data > sample.txt
                ...
                rm sample.txt
                #Replacing the  force_field_mixing_rules.def from the Replacement Folder
                cd ~/RASPA/simulations/share/raspa/forcefield/Replacement/
                cp force_field_mixing_rules.def ../AlchemicalFF/
                cd -
